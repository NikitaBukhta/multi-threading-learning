#include <iostream>
#include <thread>
#include <mutex>
#include <shared_mutex>
#include <chrono>
#include <vector>

using namespace std;
using namespace std::literals;

shared_mutex the_mutex;
//mutex the_mutex;

void write(int i) {
    /* All threads get lock in this mutex!
     * They are waiting for end of this function;
     */
	lock_guard<shared_mutex> lg(the_mutex);
	//lock_guard<mutex> lg(the_mutex);
	cout << "Write thread " << i << " with exclusive lock" << endl;
	this_thread::sleep_for(2s);
}

void read(int i) {
    /* Any thread can use this function, while exclusive lock 
     * is not used. When exclusive lock used (lock_guard in this
     * example), working of all threads that used shared_lock<shared_mutex>
     * stop. Waiting until exclusive lock is not unlocked;
     */
	shared_lock<shared_mutex> sl(the_mutex);
	//unique_lock<mutex> sl(the_mutex);
	cout << "Read thread " << i << " with shared lock" << endl;
}

int main() {
	vector<thread> threads;
	
	for (int i = 0; i < 5; ++i)
		threads.push_back(thread{read, i});
	
	threads.push_back(thread{write, 5});
	threads.push_back(thread{write, 6});
	
	for (int i = 0; i < 10; ++i)
		threads.push_back(thread{read, i+7});
	
	for (auto& t : threads)
		t.join();
}